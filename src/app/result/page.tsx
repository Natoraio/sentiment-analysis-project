...
const ResultPage: React.FC = () => {
  ...
  return (
    <div >
      ...
      <button className=" text-white px-4 py-1 rounded hover:{bg-white}" onClick={() => window.location.href = '/'}>
          Back to Home
        </button>
      ...
      <main className="px-60 py-5">
        ...
        <div className="border rounded-lg p-4">
            <p>
              {result.text.split(' ').map((word, index) => {
                const sentiment = result.sentiments.find(s => s.word === word.toLowerCase());
                const color = sentiment ? getColor(sentiment.sentiment, sentiment.weight) : 'black';
                return (
                  <span key={index} style={{ color, fontWeight: sentiment ? 'bold' : 'normal' }}>
                    {word}{' '}
                  </span>
                );
              })}
        ...
        <div className='flex items-center border rounded-lg p-4'>
          ...
          <h2 className='text-3xl font-semibold ml-10'style={{ color: getOverallSentimentColor(result.overallSentiment) }}> 
            {result.overallSentiment} ({result.confidenceScore})</h2>
        </div>
