...
const TextField: React.FC<IProps> = (props) => {
  ...
  return (
    <div className="relative w-full mb-2">
      <textarea
        className={`w-full p-2 bg-[#F4F5FF] rounded-3xl text-sm border-2 border-[#5E62CC] focus:outline-none placeholder-gray-500 ${wordCount > 80 ? 'border-red-500' : ''}`}
        value={props.state}
        onChange={handleInputChange}
        placeholder="Copy and paste your text for sentiment analysis here"
        style={{ height: '300px', color: props.state ? 'black' : 'gray' }}
      />
      <p className={`text-sm absolute right-2 ${wordCount > 80 ? 'text-red-500' : 'text-gray-500'}`}>
        {wordCount} / 80 words
      </p>
    </div>
  );
